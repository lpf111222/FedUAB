...
class FedProxClient(client_base.ClientBase):
    ...
    def local_training(self, old_model_parameters):
        ...
        for e in range(self.conf["local_epochs"]):
            ...
            for batch_id, batch in enumerate(self.train_data_loader):
                ...
                loss = F.cross_entropy(logits, target, reduction='mean').clamp(max=100.0)
                # The change of the parameter is regarded as the loss
                proximal_term = 0.0
                for name, param in self.local_model.named_parameters():
                    proximal_term += (param - old_model_parameters[name]).norm(2)
                proximal_term *= 0.01
                loss += proximal_term
                loss.backward()
                optimizer.step()
...
class FedProxServer(server_base.ServerBase):
    def __init__(self, conf, train_dataset, eval_dataset, clients_distribution, train_clients_index, eval_clients_index, MonteCarlo_times=1):
        ...
        for i in range(conf["num_client"]):
            self.clients.append(
                FedProxClient(self.global_model, conf, train_dataset, eval_dataset, clients_distribution[i],
